...
if (isset($_SESSION['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            $analisar_gpon = isset($_POST['analisar_gpon']) ? 1 : 0; // Verifica se o checkbox está marcado

            // Atualiza os dados na tabela gpon_olts
            $sql = "UPDATE gpon_olts SET analisar_gpon = :analisar_gpon, active = :active, olt_name = :oltName, city = :cidadeOLT, olt_username = :usuarioIntegracao, olt_password = :senhaIntegracao WHERE id = :id";

            // Prepara e executa a declaração preparada
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':analisar_gpon', $analisar_gpon, PDO::PARAM_INT);
