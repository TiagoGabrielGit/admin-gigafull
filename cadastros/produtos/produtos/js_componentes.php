...
<script>
    $("#btnEditarComponentes").click(function() {
        var dadosEditar = $("#componentesEditarForm").serialize();

        $.post("processa/editar_componentes.php", dadosEditar, function(retornaEditar) {
            $("#msgEditarComponentes").slideDown('slow').html(retornaEditar);

            //Apresentar a mensagem leve
            retirarMsgEditarComponentes();
        });
    });

    //Retirar a mensagem após 1700 milissegundos
    function retirarMsgEditarComponentes() {
        setTimeout(function() {
            $("#msgEditarComponentes").slideUp('slow', function() {});
        }, 1700);
    }
</script>
...
<script>
    ...
    const patrimonioInput = document.getElementById("patrimonioComponente");

    // Define o prefixo do patrimônio como "PATR_T"
    const prefixoPatrimonio = "PATR_C";
