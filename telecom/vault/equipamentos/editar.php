...
if ($rowCount_permissions_submenu > 0) {
    ...
    $sql_credenciais_equipamento =
        ...
        credequip.usuario_id as user_criador,
        ...
        FROM credenciais_equipamento as credequip
        LEFT JOIN equipamentospop as eqp ON eqp.id = credequip.equipamento_id
        LEFT JOIN empresas as emp ON emp.id = credequip.empresa_id
        LEFT JOIN usuarios as u ON u.id = credequip.usuario_id
        LEFT JOIN pessoas as p ON u.pessoa_id = p.id
        WHERE credequip.id = '$id'";

    $resultado = mysqli_query($mysqli, $sql_credenciais_equipamento);
    ...
    if ($row['cred_priv'] == 1) {
        ...
        $sql_check_perm_user = "SELECT * FROM credenciais_equipamento_privacidade_usuario WHERE credencial_id = :id AND usuario_id = :userId";
        ...
        $sql_check_perm_equipe = "SELECT * FROM credenciais_equipamento_privacidade_equipe WHERE credencial_id = :id AND equipe_id IN (SELECT equipe_id FROM equipes_integrantes WHERE integrante_id = :userId)";
